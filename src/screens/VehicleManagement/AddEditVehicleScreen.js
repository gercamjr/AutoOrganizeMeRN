...
import { ScreenWrapper, AppTextInput, AppButton, Card } from '../../components/common'; // Reverted to barrel import
...
const AddEditVehicleScreen = () => {
  ...
  useEffect(() => {
    navigation.setOptions({ title: isEditing ? `Edit Vehicle for ${customerName}` : `Add Vehicle for ${customerName}` });
    if (isEditing) {
      loadVehicleData();
    }
  ...
  if (isLoading && isEditing) { // Show loading indicator only when editing and fetching data
    return (
      <ScreenWrapper>
